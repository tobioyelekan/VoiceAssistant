...
class Sent : Fragment() {
    ...
    private fun speakMessage(smsData: SmsData) {
        ((activity?.applicationContext as Talk)).textToSpeech?.speak("message to ${smsData.address}",
    ...
    inner class LoadSms : AsyncTask<String, String, List<SmsData>>() {
        ...
        override fun doInBackground(vararg params: String?): List<SmsData>? {
            ...
            val cursor = activity?.contentResolver?.query(Telephony.Sms.Sent.CONTENT_URI, null, null, null,
